...
namespace Scenes {
    void UseScene0(Camera &camera, shared_ptr<HittableList> &world, shared_ptr<HittableList> &lights, glm::vec3 &bg_color) {
        ...
        bg_color = glm::vec3(0.5f, 0.7f, 1.0f);
    }

    void UseScene1(Camera &camera, shared_ptr<HittableList> &world, shared_ptr<HittableList> &lights, glm::vec3 &bg_color) {
        world = std::make_shared<HittableList>();
        lights = std::make_shared<HittableList>();

        world->AddHittable(std::make_shared<Mesh>(
                fs::current_path().parent_path() / "assets" / "mesh_triangle" / "cornell_box" / "cbox_luminaire.obj",
                std::make_shared<Lambertian>(glm::vec3(0.73f)),
                Transformation::FromSize(glm::vec3(0.01f))
        ));
        world->AddHittable(std::make_shared<Mesh>(
                fs::current_path().parent_path() / "assets" / "mesh_triangle" / "cornell_box" / "cbox_floor.obj",
                std::make_shared<Lambertian>(glm::vec3(0.73f)),
                Transformation::FromSize(glm::vec3(0.01f))
        ));
        world->AddHittable(std::make_shared<Mesh>(
                fs::current_path().parent_path() / "assets" / "mesh_triangle" / "cornell_box" / "cbox_ceiling.obj",
                std::make_shared<Lambertian>(glm::vec3(0.73f)),
                Transformation::FromSize(glm::vec3(0.01f))
        ));
        world->AddHittable(std::make_shared<Mesh>(
                fs::current_path().parent_path() / "assets" / "mesh_triangle" / "cornell_box" / "cbox_back.obj",
                std::make_shared<Lambertian>(glm::vec3(0.73f)),
                Transformation::FromSize(glm::vec3(0.01f))
        ));
        world->AddHittable(std::make_shared<Mesh>(
                fs::current_path().parent_path() / "assets" / "mesh_triangle" / "cornell_box" / "cbox_greenwall.obj",
                std::make_shared<Lambertian>(glm::vec3(0.12f, 0.45f, 0.15f)),
                Transformation::FromSize(glm::vec3(0.01f))
        ));
        world->AddHittable(std::make_shared<Mesh>(
                fs::current_path().parent_path() / "assets" / "mesh_triangle" / "cornell_box" / "cbox_redwall.obj",
                std::make_shared<Lambertian>(glm::vec3(0.65f, 0.05f, 0.05f)),
                Transformation::FromSize(glm::vec3(0.01f))
        ));
        world->AddHittable(std::make_shared<Mesh>(
                fs::current_path().parent_path() / "assets" / "mesh_triangle" / "cornell_box" / "cbox_smallbox.obj",
                std::make_shared<Lambertian>(glm::vec3(0.73f)),
                Transformation::FromSize(glm::vec3(0.01f))
        ));
        world->AddHittable(std::make_shared<Mesh>(
                fs::current_path().parent_path() / "assets" / "mesh_triangle" / "cornell_box" / "cbox_largebox.obj",
                std::make_shared<Lambertian>(glm::vec3(0.73f)),
                Transformation::FromSize(glm::vec3(0.01f))
        ));
